...
public class Intersection_1 {
	...
	public static void main(String[] args) {
		...
		p_o1_out.Value = new TransferOperation("localhost", "port", "u"); // to be set
		pn.PlaceList.add(p_o1_out);

		DataTransfer p_o2_out = new DataTransfer();
		p_o2_out.SetName("p_o2_out");
		p_o2_out.Value = new TransferOperation("localhost", "port", "u"); // to be set
		pn.PlaceList.add(p_o2_out);

		DataTransfer p_o3_out = new DataTransfer();
		p_o3_out.SetName("p_o3_out");
		p_o3_out.Value = new TransferOperation("localhost", "port", "u"); // to be set
...
//		grdT4out.Activations.add(new Activation(t4out, "P_s3",TransitionOperation.SendOverNetwork, "p_o3_out"));
		// the 4th just exit from model (consumed)
